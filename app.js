...
(function() {
  ...
  var _versionChecking = false;
  var _versionNotified = false;

  // 최초 로드 시 현재 app.js ETag 저장
  function initVersionTag() {
    fetch('/app.js', { method: 'HEAD', cache: 'no-cache' })
      .then(function(res) {
        var etag = res.headers.get('etag') || res.headers.get('last-modified') || '';
        if (etag) _versionEtag = etag;
      })
      .catch(function() {});
  }
  ...
  function checkVersion() {
    if (_versionChecking || _versionNotified || !_versionEtag) return;
    _versionChecking = true;
    fetch('/app.js', { method: 'HEAD', cache: 'no-cache' })
      .then(function(res) {
        var etag = res.headers.get('etag') || res.headers.get('last-modified') || '';
        if (etag && etag !== _versionEtag) {
          ...
          toast.innerHTML = [
            ...
            '<button onclick="location.reload()" style="',
              ...
              'white-space:nowrap',
            '">지금 새로고침</button>',
            '<button onclick="document.getElementById('version-update-toast').remove()" style="',
      ...
      .catch(function() {})
      .finally(function() { _versionChecking = false; });
  }

  // 페이지 로드 완료 후 초기 ETag 저장
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initVersionTag);
  } else {
    initVersionTag();
  }
