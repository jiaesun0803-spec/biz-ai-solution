...
function updateDashboardReports() {
    ...
    // ★ 최근 보고서 목록 ★
    if (!reports.length) {
        listEl.innerHTML = '<div class="empty-state">최근 생성된 보고서가 없습니다.</div>';
        return;
    }
    const typeIcon = (type) => {
        const icons = { '경영진단':'📈', '재무진단':'💰', '사업계획서':'💡', '정책자금매칭':'🎯', '상권분석':'🏪', '마케팅제안':'📢' };
        return icons[type] || '📄';
    };
    listEl.innerHTML = [...reports].reverse().slice(0, 5).map(r => `
        <div class="recent-report-item">
            ...
            <div>
                <div class="report-item-title">${r.type === '경영진단' ? '경영진단보고서' : r.title} - ${r.company}</div>
...
/* =========================================================
   ★ 표지 HTML 생성
   - 전년도 매출: y25 (25년도 매출) 사용
   - 금년 예상 매출: () 부가설명 제거
   - th/td: 가운데 정렬 (CSS에서 처리)
========================================================= */
        ...
        <div class="cover-middle">
            ...
            <div class="cover-table">
                <table>
                    ...
                    <tr>
                        <th>전년도 매출</th>
                        <td>${formatKoreanCurrency(safeRev.y25)}</td>
                        <th>금년 예상 매출</th>
                        <td>${formatKoreanCurrency(expectedCur)}</td>
                    </tr>
                </table>
            </div>
        </div>
...
function buildCompanyOverviewTable(companyData, rev) {
    ...
    return `<table class="overview-company-table">
        ...
        <tr>
            <th>전년도 매출 (25년)</th><td>${formatKoreanCurrency(safeRev.y25)}</td>
            <th>금년 예상 매출</th><td>${formatKoreanCurrency(exp)}</td>
        </tr>
    </table>`;
}
...
[진단 가중치]
1. 경영진단 개요: 3~4항목 | 2. 재무 현황 분석: 가중치 15% → li 3~4개
3. 전략 및 마케팅 분석: 가중치 25% → li 6~8개 | 4. 인사/조직: 가중치 20% → li 5~6개
5. 운영/생산: 가중치 20% → li 5~6개 | 6. IT/디지털 및 정부지원: 가중치 20% → li 각 2~3개
7. 개선 방향 및 로드맵: 5~6항목
${COMMON_RULES}
...
${COMMON_RULES}출력목적: ${version==='client'?'정식 제출용':'초안/검토본'}
[기업데이터] ${JSON.stringify({...cData,rawData:undefined,revenueData:undefined,매출데이터:fRev},null,2)}`},
...
${COMMON_RULES}출력목적: ${version==='client'?'기업전달용':'컨설턴트용(리스크포함)'}
[기업데이터] ${JSON.stringify({...cData,rawData:undefined,revenueData:undefined,매출데이터:fRev},null,2)}`},
