...
// Public suffix domains where we should NOT set a parent domain cookie
// (browsers block cookies on these domains for security)
 ...
 * Returns undefined for public suffix domains (railway.app, vercel.app, etc.)
 */
function getParentDomain(hostname: string): string | undefined {
  ...
  // Get the parent domain (last 2 parts)
  const parentDomain = parts.slice(-2).join(".");

  // Don't set domain for public suffix domains (browsers block these)
  if (PUBLIC_SUFFIX_DOMAINS.has(parentDomain)) {
    return undefined;
  }

  // Also check for 3-part public suffixes like "up.railway.app"
  if (parts.length >= 3) {
    const threePartSuffix = parts.slice(-3).join(".");
    if (PUBLIC_SUFFIX_DOMAINS.has(threePartSuffix)) {
      return undefined;
    }
  }
