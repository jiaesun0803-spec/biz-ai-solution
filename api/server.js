...
app.post('/api/auth/login', async (req, res) => {
  ...
  // 비밀번호 검증 (bcrypt 또는 평문 지원)
  let valid = false;
  if (user.pw.startsWith('$2')) {
    valid = await bcrypt.compare(pw, user.pw);
  } else {
    valid = (pw === user.pw); // 초기 관리자 계정 평문 지원
  }
...
app.get('/api/companies', authMiddleware, async (req, res) => {
  const { data, error } = await supabase.from('companies').select('*').eq('user_id', req.user.id).order('updated_at', { ascending: false });
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});
...
app.put('/api/companies/:id', authMiddleware, async (req, res) => {
  const { data, error } = await supabase.from('companies').update({ ...req.body, updated_at: new Date().toISOString() }).eq('id', req.params.id).eq('user_id', req.user.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});
...
app.get('/api/reports', authMiddleware, async (req, res) => {
  const { data, error } = await supabase.from('reports').select('id,type,title,created_at,company_id').eq('user_id', req.user.id).order('created_at', { ascending: false });
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});
...
app.post('/api/reports', authMiddleware, async (req, res) => {
  const { type, title, html, data: reportData, company_id } = req.body;
  const { data, error } = await supabase.from('reports').insert({
    user_id: req.user.id, company_id: company_id || null,
    type, title, html: html || '', data: reportData || {}
  }).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});
...
app.get('/api/reports/:id', authMiddleware, async (req, res) => {
  const { data, error } = await supabase.from('reports').select('*').eq('id', req.params.id).eq('user_id', req.user.id).single();
  if (error) return res.status(404).json({ error: '보고서를 찾을 수 없습니다.' });
  res.json(data);
});
...
app.listen(PORT, () => console.log(`BizConsult API 서버 실행 중: http://localhost:${PORT}`));
