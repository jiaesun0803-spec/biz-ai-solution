...
// Auto-derive API base URL from Metro/Packager URL for native (iOS/Android) OAuth
// Metro URL: https://8081-sandboxid.region.domain -> API URL: https://3000-sandboxid.region.domain
const metroUrl =
  process.env.EXPO_PACKAGER_PROXY_URL ||
  process.env.EXPO_PUBLIC_METRO_URL ||
  "";
if (metroUrl && !process.env.EXPO_PUBLIC_API_BASE_URL) {
  const apiUrl = metroUrl.replace(/^(https?:\/\/)8081-/, "$13000-").replace(/\/+$/, "");
  if (apiUrl !== metroUrl) {
    process.env.EXPO_PUBLIC_API_BASE_URL = apiUrl;
    process.env.EXPO_PUBLIC_METRO_URL = metroUrl;
    console.log("[load-env] Auto-derived API URL:", apiUrl);
  }
}
